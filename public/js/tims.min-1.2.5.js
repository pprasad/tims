...
tims.createRows=function(dataGrid){
 ...
 row.append("<td>"+(dataGrid.passed!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+dataGrid.val+"',"+STATUS_TYPE.passed+")>"+dataGrid.passed+"</a>":dataGrid.passed)+"</td><td>"+(dataGrid.pending!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+dataGrid.val+"',"+STATUS_TYPE.pending+")>"+dataGrid.pending+"</a>":dataGrid.pending)+"</td>");
 row.append("<td>"+(dataGrid.failed!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+dataGrid.val+"',"+STATUS_TYPE.failed+")>"+dataGrid.failed+"</a>":dataGrid.failed)+"</td><td>"+(dataGrid.blocked!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+dataGrid.val+"',"+STATUS_TYPE.blocked+")>"+dataGrid.blocked+"</a>":dataGrid.blocked)+"</td>");
 row.append("<td>"+(dataGrid.dropped!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+dataGrid.val+"',"+STATUS_TYPE.dropped+")>"+dataGrid.dropped+"</a>":dataGrid.dropped)+"</td><td>"+(dataGrid.passedexp!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+dataGrid.val+"',"+STATUS_TYPE.passedexp+")>"+dataGrid.passedexp+"</a>":dataGrid.passedexp)+"</td>");
 row.append("<td>"+(dataGrid.executed!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+dataGrid.val+"',"+STATUS_TYPE.executed+")>"+dataGrid.executed+"</a>":dataGrid.executed)+"</td><td>"+(dataGrid.total!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+dataGrid.val+"',"+STATUS_TYPE.total+")>"+dataGrid.total+"</a>":dataGrid.total)+"</td>");
row.append("<td>"+dataGrid.quality+"%<td>"+(dataGrid.defects!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+dataGrid.val+"',"+STATUS_TYPE.defects+")>"+dataGrid.defects+"</a>":dataGrid.defects)+"</td>");
 $("#dataGrid").treetable("loadBranch",node,row);
...
tims.loadChildRows=function(parent,object,node){
  ...
  $(object).each(function(key,obj){
		 ...
		 /*<td>"+(obj.pending!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+obj.val+"',"+STATUS_TYPE.pending+")>"+obj.pending+"</a>":obj.pending)+"</td>");*/
		 ...
		 row.append("<td>"+(obj.passed!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+obj.val+"',"+STATUS_TYPE.passed+")>"+obj.passed+"</a>":obj.passed)+"</td><td>"+(obj.pending!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+obj.val+"',"+STATUS_TYPE.pending+")>"+obj.pending+"</a>":obj.pending)+"</td>");
         row.append("<td>"+(obj.failed!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+obj.val+"',"+STATUS_TYPE.failed+")>"+obj.failed+"</a>":obj.failed)+"</td><td>"+(obj.blocked!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+obj.val+"',"+STATUS_TYPE.blocked+")>"+obj.blocked+"</a>":obj.blocked)+"</td>");
         row.append("<td>"+(obj.dropped!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+obj.val+"',"+STATUS_TYPE.dropped+")>"+obj.dropped+"</a>":obj.dropped)+"</td><td>"+(obj.passedexp!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+obj.val+"',"+STATUS_TYPE.passedexp+")>"+obj.passedexp+"</a>":obj.passedexp)+"</td>");
		 row.append("<td>"+(obj.executed!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+obj.val+"',"+STATUS_TYPE.executed+")>"+obj.executed+"</a>":obj.executed)+"</td><td>"+(obj.total!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+obj.val+"',"+STATUS_TYPE.total+")>"+obj.total+"</a>":obj.total)+"</td>");
		 row.append("<td>"+obj.quality+"%</td><td>"+(obj.defects!=0?"<a class='/link-normal/' href=javascript:tims.href_event('"+obj.val+"',"+STATUS_TYPE.defects+")>"+obj.defects+"</a>":obj.defects)+"</td>");
...
tims.resultTable=function(){
   resultDataset=$('#resultDataset').DataTable({
		 ...
		 "createdRow": function ( row, data, index ) {
			...
			$('td', row).eq(3).html("<img src='images/status/blocked.gif'><span> </span>"+data.Status);
			}
...
tims.exportExcel=function(_targetObjId,_targetObjId1){
  var report='<table border="1" width="100%">';
  report+='<tr style="background-color:orange"><td><b>Folders</b></td><td><b>Passed</b></td>';
  report+='<td><b>Pending</b></td><td><b>Failed</b></td>';
  report+='<td><b>Blocked</b></td><td><b>Dropped</b></td>';
  report+='<td><b>PassW/X</b></td><td><b>Executed</b></td>';
  report+='<td><b>Total</b></td><td><b>Quality</b></td><td><b>Defects</b></td></tr>';
  $('#'+_targetObjId+"  tbody tr").each(function(key,row){
       $(this).each(function(key,col){
	       var  row='<tr>'; 
	        $(this).find('td').each(function(key,cell){
			      var col='<td>';
				  var ele='';
			      if($(this).find('span:eq(1)').is('span')){
			          var ele=$(this).find('span:eq(1)').html();
                      ele=ele.replace('<span></span>','');	
				  }
				  else if(!$(this).find('a').is('a')){
				        ele=$(this).html();
				  }
				  else if($(this).find('a').is('a')){
				       ele=$(this).find('a').html();
				  }
                 col+='&nbsp;&nbsp;'+ele+'</td>';
				 row+=col;				  
			})
  ...
  report+='</table>';
  $("#"+_targetObjId1).attr("href",'data:Application/vnd.ms-excel,'+ encodeURIComponent(report))[0].click()
}
